document.addEventListener("DOMContentLoaded", () => {
    ...
    const stepInfo = {
        proposals: {
            title: "Step 1: Region Proposal Generator (RoIs)",
            desc: "A contour & MSER search extracts bounding boxes likely containing digits (seeds), then generates overlapping candidate boxes (blue). In Fast R-CNN, ALL proposals are processed together in parallel through RoI pooling."
        },
        regression: {
            title: "Step 2: Bounding Box Regression",
            desc: "The regression head predicts offset coordinates (scaling and shifting values) for each proposal. The yellow boxes show the adjusted boxes, and the dashed arrows illustrate how the model shifts them to tightly envelope the digits."
        },
        detections: {
            title: "Step 3: Final NMS Digit Detections",
            desc: "Proposals predicted as background are discarded. The remaining digit proposals are thresholded, and Non-Maximum Suppression (NMS) removes duplicate overlapping predictions, yielding the final tight green detections."
        }
    };
    ...
    async function runDetection() {
        ...
        try {
            ...
            statLatency.textContent = `${data.metrics.fast_rcnn_latency_ms} ms`;
            statRcnnLatency.textContent = `${data.metrics.est_rcnn_latency_ms} ms`;
            statSpeedup.textContent = `${data.metrics.speedup_ratio}x`;

            // Load and draw the normalized image returned by the server on the visualization canvas
            ...
            btnDetect.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Run Fast R-CNN Detection';
        }
    }
    ...
    function renderOverlays() {
        ...
        if (activeStep === "proposals") {
            ...
            // We use an SVG layer to draw vector arrows
            ...
            defs.innerHTML = `
                <marker id="arrowhead" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
                    <polygon points="0 0, 6 3, 0 6" fill="#ffd166" />
                </marker>
            `;
